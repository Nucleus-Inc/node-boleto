...
exports.addTrailingSpaces = function(string, length) {
  string = string.toString();
...
exports.formatDate = function(date, format) {
  ...
  if (format) {
    var day = exports.addTrailingZeros(date.getDate(), 2);
    var month = exports.addTrailingZeros(date.getMonth() + 1, 2);
    var year = date.getYear();
    var fullYear = date.getFullYear();

    format = format.replace("dd", day.toString());
    format = format.replace("mm", month.toString());
    format = format.replace("aaaa", fullYear.toString());
    format = format.replace("aa", year.toString());
